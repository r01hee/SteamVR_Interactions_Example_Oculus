...
public class SteamVR_Controller
{
	...
	public class Device
	{
		...
		public void Update()
		{
			if (Time.frameCount != prevFrameCount)
			{
				...
				GetControllerStateWithPose(index, ref state, ref pose);
				UpdateHairTrigger();
			}
		}

		private void GetControllerStateWithPose(uint index, ref VRControllerState_t pControllerState, ref TrackedDevicePose_t pTrackedDevicePose)
		{
			ulong buttonPressed = 0;
			...
			if (index == 0) {
				...
				if (OVRInput.Get(OVRInput.RawButton.LHandTrigger)) {
					buttonPressed |= ButtonMask.Grip;
				}
				if (OVRInput.Get(OVRInput.Button.PrimaryThumbstick)) {
					buttonPressed |= ButtonMask.Touchpad;
				}
				if (OVRInput.Get(OVRInput.RawButton.LIndexTrigger)) {
					buttonPressed |= ButtonMask.Trigger;
				}
			} else {
			...
			state.rAxis0.x = OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, controllerType);
			state.rAxis1.x = OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger, controllerType);

			Vector3 velocity = OVRInput.GetLocalControllerVelocity(controllerType);
			pose.vVelocity.v0 = velocity[0];
			pose.vVelocity.v1 = velocity[1];
			pose.vVelocity.v2 = -velocity[2];
			Vector3 angularVelocity = OVRInput.GetLocalControllerVelocity(controllerType);
			pose.vAngularVelocity.v0 =  angularVelocity[0];
			pose.vAngularVelocity.v1 =  angularVelocity[1];
			pose.vAngularVelocity.v2 =  angularVelocity[2];

			state.ulButtonPressed =  buttonPressed;
			...
			pTrackedDevicePose.eTrackingResult = ETrackingResult.Running_OK;
		}
		...
		void UpdateHairTrigger()
		{
			...
			Debug.Log("HairTrigger: " + hairTriggerState + " / " + value + " " + hairTriggerLimit);
		}
		...
		int relativeTo = (int)OpenVR.k_unTrackedDeviceIndex_Hmd) // use -1 for absolute tracking space
	{
		switch (relation)
		{
			case SteamVR_Controller.DeviceRelation.Leftmost:
				return 0;
			case SteamVR_Controller.DeviceRelation.Rightmost:
				return 1;
            default:
				return -1;
		}
